...
class InstrumentoGeneralMIDI:
    numero: int

    def __post_init__(self) -> None:
        if not 0 <= self.numero <= 127:
            raise ValueError("Instrumento General MIDI deve estar entre 0 e 127.")

    @property
    def nome(self) -> str:
        return NOMES_GENERAL_MIDI[self.numero]

    @classmethod
    def validar_numero(cls, numero: int) -> int:
        return cls(numero).numero
